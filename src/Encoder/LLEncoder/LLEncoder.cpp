...
namespace ProVivid {
    ...
    int32_t LLEncoder::DetermineMBdeltaQP(uint8_t subPicQP, uint32_t pos, uint32_t stride) {
        uint32_t mean8x8 = 0;
        uint32_t pixel = 0;
        ...
        for (uint8_t yPos = 0; yPos < 8; ++yPos) {
            for (uint8_t xPos = 0; xPos < 8; ++xPos) {
                int blkXidx = xPos / 4;
                int blkYidx = yPos / 4;
                pixel = llBandPic_->at(Y)->at(picIndex + xPos);
        ...
        //printf(" var_min : %d", varMin);
        int mbQpDeltaC = 0;
        if (isSubpicBoud && (var8x8 < 10000)) {
            int curMbQP;
            Clip(qp_[Y] + mbQpDeltaA + mbQpDeltaB, curMbQP, 0, 39);
            //curMbQP = qp_[Y] + mbQpDeltaA + mbQpDeltaB + mbQpDeltaC;
            if (curMbQP > 8)
                mbQpDeltaC = 8 - qp_[Y] - mbQpDeltaA - mbQpDeltaB;
        }
    ...
    void LLEncoder::LLEncode(Bitstream* bitstreamVlcLl, EncodingEnvironment* eeCabacLl, TextureInfoContexts* texCtx, MotionInfoContexts* motCtx, int frameType, bool qpDeltaEnable)
    {
        ...
        for (mbY_ = 0; mbY_ < maxHeightMb; mbY_++) {
            ...
            for (mbX_ = 0; mbX_ < maxWidthMb; mbX_++) {
                ...
                if (qpDeltaEnable) {
                    ...
                    Clip(qp_[Y] + DetermineMBdeltaQP(qp_[Y], pos, llBandWidth_), curMbQP, 0, 39);
