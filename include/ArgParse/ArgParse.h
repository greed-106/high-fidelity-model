...
namespace argparse {
    ...
    struct Entry {
        ...
        void set_datap_value(T1 v) {
            datap->convert(v);
        }

        bool set_by_user() {
            return is_set_by_user;
        }
    ...
    public:
        ...
        std::vector<std::shared_ptr<Entry>>& get_all_entries() {
            return all_entries;
        }
        ...
        std::string print() {
            ...
            for (const auto &entry : all_entries) {
                ...
                arg_str << setw(21) << entry->_get_keys() + snip << " : " << (entry->is_set_by_user? bold(entry->value_.value_or("null")) : entry->value_.value_or("null")) << endl;
            }

            for (const auto &[subcommand, subentry] : subcommand_entries) {
                if (subentry->subargs->is_valid) {
                    arg_str << endl << "--- Subcommand: " << subcommand << endl;
                    arg_str << subentry->subargs->print();
                }
            }
            return arg_str.str();
        }
